...
def index_cv(candidate_id: str, name: str, text: str, source: str = "",
             source_url: str = "", synthetic: bool = False) -> int:
    ...
    col.upsert(
        ...
        metadatas=[{
            ...
            # Whether this person was generated. Recorded on the row rather
            # than inferred from the collection name, because a name is a
            # config value and this is a safety marker: the UI used to show
            # its "not real people" banner only when the collection matched
            # /demo|test|sample/, so the local dev corpus -- named "cvs" --
            # served forty invented people, with phone numbers and visa
            # status, wearing no warning at all. Travelling with the data
            # means the marker survives a rename, a copy, and an export.
...
def index_corpus(directory: Path, synthetic: bool = False) -> Dict[str, int]:
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    ap.add_argument("--synthetic", action="store_true",
                    help="mark every CV in this corpus as generated, not a real "
                         "person. Inferred automatically for corpus/synthetic; "
                         "pass it explicitly for any other invented set.")
    ...
    # corpus/synthetic is generated by corpus/generate.py, so nobody should
    # have to remember the flag for the one corpus that always needs it.
    synthetic = args.synthetic or args.corpus.resolve().name == "synthetic"
    label = " (marked synthetic)" if synthetic else ""
    print(f"indexing {args.corpus}{label} ...")
    stats = index_corpus(args.corpus, synthetic=synthetic)
